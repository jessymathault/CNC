...
void ZupISR(void){
  if(digitalRead(37) == LOW || digitalRead(40) == LOW){
    ZupHit = true;  
  }
  else{
    ZupHit = false;
...
void ZdownISR(void){
  if(digitalRead(38) == LOW || digitalRead(39) == LOW){
    ZdownHit = true;  
  }
  else{
    ZdownHit = false;
...
boolean LimitSwitches::pxLimit(){
  return XrightHit;
}
boolean LimitSwitches::nxLimit(){
  return XleftHit;
}
boolean LimitSwitches::pyLimit(){
  return YtopHit;
}
boolean LimitSwitches::nyLimit(){
  return YbottomHit;
}
boolean LimitSwitches::pzLimit(){
  return ZupHit;
}
boolean LimitSwitches::nzLimit(){
  return ZdownHit;
}
